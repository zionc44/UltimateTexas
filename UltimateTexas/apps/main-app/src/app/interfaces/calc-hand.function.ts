...
export interface ExpressBonusHand {
    rank: ExpressBonusRank;
    cards: Card[]; // Cards that contribute to the hand
    handDescription: string; // Textual description of the hand rank
}
...
const isFivePictureCards = (cards: Card[]): boolean => {
    let isFivePictures = true;
    cards.forEach(card => {
        if (card.cardValue != 11 && card.cardValue != 12 && card.cardValue != 13) {
            isFivePictures = false
        }
    })
    return isFivePictures;
};

// Helper function: Ace - King - Queen
const isAceKingQueen = (cards: Card[]): boolean => {
    const sorted = sortCardsByValue(cards);
    const uniqueCards = Array.from(new Map(sorted.map(card => [card.cardValue, card])).values());
    const aceKingQueen = [14, 13, 12];
    return aceKingQueen.every(value => uniqueCards.some(card => card.cardValue === value))
};

export const isAceKing = (cards: Card[]): boolean => {
    const sorted = sortCardsByValue(cards);
    const uniqueCards = Array.from(new Map(sorted.map(card => [card.cardValue, card])).values());
    const aceKingQueen = [14, 13];
    return aceKingQueen.every(value => uniqueCards.some(card => card.cardValue === value))
};

const isSameFiveColorCards = (cards: Card[]): boolean => {
    let numOfRed = 0;
    let numofBlack = 0;
    cards.forEach(card => {
        if (card.cardSign == CardSign.CLUB || card.cardSign == CardSign.SPADE) {
            numofBlack++;
        } else {
            numOfRed++;
        }
    ...
    return numOfRed == 5 || numofBlack == 5;
};
...
export function calculateExpressBonus(cards: Card[]): ExpressBonusHand {
    if (cards.length != 5) {
        throw new Error("For NOVO bonus, the number of cards must be 5.");
    }
    ...
    if (flush) {
        ...
        if (straightFlush) {
            return {
                rank: straightFlush[0].cardValue === 14 ? ExpressBonusRank.RoyalFlush : ExpressBonusRank.StraightFlush,
                cards: straightFlush,
                handDescription: straightFlush[0].cardValue === 14 ? "Royal Flush" : `Straight Flush to ${cardValueToString(straightFlush[0].cardValue)}`,
            };
        }
    ...
    // Check for 5 picture cards
    if (isFivePictureCards(cards)) {
        return {
            rank: ExpressBonusRank.FivePictureCards,
            cards: sortedCards,
            handDescription: `Five Picture Cards`,
        };
    }
    ...
    if (trips.length > 0) {
        ...
        if (pairs.length > 0) {
            pairs.sort((a, b) => b[0].cardValue - a[0].cardValue);
            return {
                rank: ExpressBonusRank.FullHouse,
                cards: [...trips[0], ...pairs[0]],
                handDescription: `Full House, ${cardValueToString(trips[0][0].cardValue)}s over ${cardValueToString(pairs[0][0].cardValue)}s`,
            };
        } else if (trips.length > 1) {
    ...
    if (straight) {
        return {
            rank: ExpressBonusRank.Straight,
            cards: straight,
            handDescription: `Straight to ${cardValueToString(straight[0].cardValue)}`,
        };
    }
    ...
    if (trips.length > 0) {
        const kickers = sortedCards.filter(card => card.cardValue !== trips[0][0].cardValue).slice(0, 2);
        return {
            rank: ExpressBonusRank.TreeOfKing,
            cards: [...trips[0], ...kickers],
            handDescription: `Three of a Kind, ${cardValueToString(trips[0][0].cardValue)}s`,
        };
    }
    ...
    if (isAceKingQueen(cards)) {
        return {
            rank: ExpressBonusRank.AceKingQueen,
            cards: sortedCards,
            handDescription: `Ace - King - Queen`,
        };
    }

    // Check same five color cards
    if (isSameFiveColorCards(cards)) {
        return {
            rank: ExpressBonusRank.FiveCradSameColor,
            cards: sortedCards,
            handDescription: `Five Card Same Color`,
        };
    }

    // Default to High Card
    return {
        rank: ExpressBonusRank.NoBonus,
        cards: sortedCards,
        handDescription: `No Express Bonus`,
    };
}
